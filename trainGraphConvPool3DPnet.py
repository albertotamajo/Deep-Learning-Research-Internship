# Built-in modules
from graphConvPool3DPnet import GraphConvPool3DPnet, XConvBatch, ShrinkingLayer, KMeansInitMostDistantFromMean
...
out_xconv1 = 5
out_xconv2 = 7
xConv1 = XConvBatch(input_feature, out_xconv1, dim, k, out_xconv1 - input_feature, d)
xConv2 = XConvBatch(out_xconv1, out_xconv2, dim, k, out_xconv2 - out_xconv1, d)
xConvLayers = [] # TODO

input_feature = dim #+ out_xconv2 # TODO
...
shrinkingLayer1 = ShrinkingLayer(mlp, 1.0, out_points, "k-means++", n_init, sigma, F, W, M, B, C, P, mlp1, mlp2)
...
mlp2 = MLP2Layers(C+P,  C + P + 5, C + P + 10, C+P)
input_feature = out_feature
shrinkingLayer2 = ShrinkingLayer(mlp, 1.0, out_points, "k-means++", n_init, sigma, F, W, M, B, C, P, mlp1, mlp2)
...
mlp2 = MLP2Layers(C+P,  C + P + 5, C + P + 10, C+P)
input_feature = out_feature
shrinkingLayer3 = ShrinkingLayer(mlp, 1.0, out_points, "k-means++", n_init, sigma, F, W, M, B, C, P, mlp1, mlp2)
...
model = GraphConvPool3DPnet(xConvLayers, shrinkingLayers, mlpClassifier)
...
# Save the model
torch.save(model, os.path.join(dir_path, "graphConvPool3DPnet.pth"))